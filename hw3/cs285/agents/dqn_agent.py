    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self.use_double_q:
                target = self.target_critic(next_obs)
                next_action = torch.argmax(target, dim = -1)
            else:
        ...
        loss = self.critic_loss(q_values, target_values) # (pred, y) 

        self.critic_optimizer.zero_grad()
        loss.backward()
        # If the gradient norm is larger than a threshold,
        # scale the gradients down so that the norm is equal to the threshold
