...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        def flatten(seq: Sequence[np.ndarray]) -> np.ndarray: 
            return np.concatenate([arr for arr in seq])
        obs = flatten(obs) 
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        if not self.use_reward_to_go:
            ...
            for i in range(len(rewards)): 
                q_values.append(np.array(self._discounted_return(rewards[i])))
        else:
            ...
            q_values = [] 
            for i in range(len(rewards)): 
                q_values.append(np.array(self._discounted_reward_to_go(rewards[i]))) 
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            if self.gae_lambda is None:
                ...
                # values should have same mean and deviation as advantages. 
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        out = []
        for t in range(T):
            out.append(np.sum(gammas[:T-t] * rewards[t:]))
        return out 
