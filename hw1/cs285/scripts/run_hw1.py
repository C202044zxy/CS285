    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            paths, envsteps_this_batch = utils.sample_trajectories(env, actor,
                                        params['batch_size'], params['ep_len'])
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          # HINT3: return correspondReplayBuffering data points from each array (i.e., not different indices from each array)
          # for imitation learning, we only need observations and actions.  
          ind = np.random.randint(0, replay_buffer.obs.shape[0], size = params["train_batch_size"])
          ind = np.random.permutation(ind) # permute along the first dimension
