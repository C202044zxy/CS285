...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def forward(self, observation: torch.FloatTensor) -> Any:
        ...
        return distributions.Normal(
                loc = self.mean_net(torch.tensor(observation, dtype = torch.float32)), 
                scale = self.logstd.exp()
            )
        # return distribution with size of (batch_size, output_dim) 
    ...
    def update(self, observations, actions):
        ...
        prediction = self.forward(torch.tensor(observations, dtype = torch.float32)) 
        loss = - prediction.log_prob(torch.tensor(actions, dtype = torch.float32)).mean() 
