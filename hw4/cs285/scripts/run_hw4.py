...
def collect_mbpo_rollout(
    ...
    for _ in range(rollout_len):
        ...
        next_ob = [mb_agent.get_dynamics_predictions(i, ob[None], ac[None]) for i in range(mb_agent.ensemble_size)] 
        next_ob = np.mean(np.array(next_ob), axis = 0).squeeze(0)
...
def run_training_loop(
    ...
    for itr in range(config["num_iters"]):
        ...
        for _ in tqdm.trange(
            ...
            samp = replay_buffer.sample(config["train_batch_size"])
            step_losses = [mb_agent.update(i, samp["observations"], samp["actions"],
                                samp["next_observations"]) for i in range(mb_agent.ensemble_size)]
