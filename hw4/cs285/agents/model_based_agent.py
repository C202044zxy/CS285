...
class ModelBasedAgent(nn.Module):
    ...
    def update(self, i: int, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        delta = next_obs - obs 
        input = (input - self.obs_acs_mean) / (self.obs_acs_std + self.eps)
        delta = (delta - self.obs_delta_mean) / (self.obs_delta_std + self.eps)
        pred = self.dynamics_models[i](input) 
        loss = self.loss_fn(pred, delta)  
    ...
    def update_statistics(self, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        input = torch.cat([obs, acs], dim = -1)
        delta = next_obs - obs 
        self.obs_acs_mean = input.mean(axis = 0) 
        self.obs_acs_std = input.std(axis = 0) 
        self.obs_delta_mean = delta.mean(axis = 0)
        self.obs_delta_std = delta.std(axis = 0) 
    ...
    def get_dynamics_predictions(
        ...
        input = torch.cat([obs, acs], dim = -1) 
        input = (input - self.obs_acs_mean) / (self.obs_acs_std + self.eps)
        pred_delta = self.dynamics_models[i](input) 
        pred_delta = pred_delta * (self.obs_delta_std + self.eps) + self.obs_delta_mean 
    ...
    def evaluate_action_sequences(self, obs: np.ndarray, action_sequences: np.ndarray):
        ...
        # (mpc_num_action_sequences, horizon, ac_dim) 

        # TODO(student): for each batch of actions in in the horizon...
        for acs in action_sequences.swapaxes(0, 1): 
            ...
            acs_ = np.tile(acs, (self.ensemble_size, 1))
            next_obs_ = next_obs.reshape(-1, self.ob_dim)  
            rewards, dones = self.env.get_reward(next_obs_, acs_)
    ...
    def get_action(self, obs: np.ndarray):
        ...
        if self.mpc_strategy == "random":
            ...
            for i in range(self.cem_num_iters):
                ...
                elite = action_sequences[indices] 
                if i == 0: 
                    elite_mean = elite.mean(axis = 0)
                    elite_std = elite.std(axis = 0)
                else: 
                    elite_mean = self.cem_alpha * elite.mean(axis = 0) + (1 - self.cem_alpha) * elite_mean
                    elite_std = self.cem_alpha * elite.std(axis = 0) + (1 - self.cem_alpha) * elite_std
            return elite_mean[0]
